...
def warehouse():
    if current_user.access_level != 'worker':
        return make_response({'Status Code': '401'})
    return warehouse_list_service()

@worker_blueprint.route('/worker/todolist', methods=['GET'])
@login_required
def todolist():
    print(current_user.access_level)
    if current_user.access_level != 'worker':
        return make_response({'Status Code':'401'})
    return worker_todolist_service(current_user.id)

@worker_blueprint.route('/worker/todolist/<task_id>/done', methods=['PUT'])
def task_done(task_id):
    if current_user.access_level != 'worker':
        return make_response({'Status Code':'401'})
    data = request.get_json()
    return task_done_service(current_user.id, task_id, data)

@worker_blueprint.route('/worker/scanner', methods=['PUT'])
def scanner():
    if current_user.access_level != 'worker':
        return make_response({'Status Code':'401'})
    data = request.get_json()
    return worker_scanner_service(current_user.id,data)
