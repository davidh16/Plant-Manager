...
from service.warehouse_manager_service import new_part_service, get_part_info_service, get_parts_list_service,\
                                            update_part_service, get_critical_parts_list
...
@warehouse_manager_blueprint.route('/warehouse_manager/critical_list', methods=['GET'])
@login_required
def critical():
    if current_user.access_level != 'warehouse_manager':
        return make_response({'Status Code':'401'})
    return get_critical_parts_list()