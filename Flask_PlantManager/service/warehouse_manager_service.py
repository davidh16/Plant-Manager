...
def get_critical_parts_list():
    critical_parts = Part.query.filter(Part.quantity <= Part.critical_quantity)
    if critical_parts:
        return retrieved_parts_to_json(critical_parts)
    return make_response({'message':'No critical parts'})